package org.mtf.index12306.biz.ticketservice.mq.consumer;
...
import org.mtf.index12306.biz.ticketservice.common.constant.TicketRocketMQConstant;
...
import org.mtf.index12306.biz.ticketservice.mq.domain.MessageWrapper;
import org.mtf.index12306.biz.ticketservice.mq.event.DelayCloseOrderEvent;
...
import static org.mtf.index12306.biz.ticketservice.common.constant.RedisKeyConstant.TRAIN_STATION_REMAINING_TICKET;
...
public class DelayCloseOrderConsumer implements RocketMQListener<MessageWrapper<DelayCloseOrderEvent>> {
    ...
    public void onMessage(MessageWrapper<DelayCloseOrderEvent> delayCloseOrderEventMessageWrapper) {
        ...
        try {
            closedTickOrder = orderRemoteService.closeTickOrder(new OrderCancelReqDTO(orderSn));
        } catch (Throwable ex) {
        ...
        if (closedTickOrder.isSuccess() && !StrUtil.equals(ticketAvailabilityCacheUpdateType, "binlog")) {
            ...
            try {
                ...
                OrderInfoRespDTO ticketOrderDetail = BeanUtil.convert(delayCloseOrderEvent, OrderInfoRespDTO.class);
                ticketOrderDetail.setPassengerDetails(BeanUtil.convert(delayCloseOrderEvent.getTrainPurchaseTicketResults(), OrderItemRespDTO.class));
